...
//: @implementation DropdownRunAgainstRectangle
@implementation DropdownRunAgainstRectangle

//: + (NSString *)bigFileMD5:(NSString *)filePath
+ (NSString *)measureEnvelope:(NSString *)filePath
{
...
+ (void)pair:(NSString *)filepath project:(void(^)(NSString *MD5))completion;
{
    ...
    dispatch_block_t block = ^(){
        ...
        NSString *md5 = [self endMember:filepath];
        //: if(completion) {
        if(completion) {
            //: completion(md5);
            completion(md5);
...
+ (NSString *)endMember:(NSString *)filepath
{
    //: NSString *md5 = nil;
    NSString *md5 = nil;
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:filepath])
    {
        ...
        unsigned long long fileSize = [self focus:filepath];
        //: if(fileSize > 200 * 1024 * 1024) {
        if(fileSize > 200 * 1024 * 1024) {
            //: md5 = [self bigFileMD5:filepath];
            md5 = [self measureEnvelope:filepath];
        //: }else {
        }else {
...
+ (unsigned long long)focus:(NSString *)filepath
{
