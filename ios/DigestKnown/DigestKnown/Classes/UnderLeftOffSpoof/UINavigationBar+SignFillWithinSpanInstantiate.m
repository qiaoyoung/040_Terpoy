...
// __CLOSE_PRINT__
//
//  UINavigationBar+SignFillWithinSpanInstantiate.m
//  NIM
//
...
//: @implementation UINavigationBar (SignFillWithinSpanInstantiate)
@implementation UINavigationBar (SignFillWithinSpanInstantiate)
...
+ (void)load{
    ...
    _dispatch_once(&onceToken, ^{
        ...
        swizzling_exchangeMethod([UINavigationBar class] ,@selector(layoutSubviews), @selector(forceTotalegrate));
    //: });
    });
}
...
- (void)treasure{
    ...
    CGPoint centerPonit = CGPointMake(self.triumphConversation * .5f, self.realm *.5f);
...
- (void)forceTotalegrate{
    //: [self swizzling_layoutSubviews];
    [self forceTotalegrate];
    //: UINavigationItem *navigationItem = [self topItem];
    UINavigationItem *navigationItem = [self topItem];
    ...
    subview.chipSurf = navigationBtnMargin;
