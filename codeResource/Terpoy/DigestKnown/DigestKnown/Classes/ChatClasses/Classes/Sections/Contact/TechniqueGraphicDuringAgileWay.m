...
- (IBAction)onDoneBtnClick:(id)sender {
    ...
    if (_selectecContacts.count == 0) {
        [ScaffoldOntoOrchestrate showMessage:LangKey(@"main_activity_choose_least_one")];
        return;
    }
...
- (AmortizationCapacityTaxonomy *)selectIndicatorView{
    ...
    _selectIndicatorView = [[AmortizationCapacityTaxonomy alloc] initWithFrame:CGRectMake(0, SCREEN_TOP_HEIGHT, SCREEN_WIDTH, 64)];
...
- (ProbeRestBrookWater *)setGroupnameView{
    if(!_setGroupnameView){
        _setGroupnameView = [[ProbeRestBrookWater alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
