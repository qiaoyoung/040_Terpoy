...
//: @class PopulateServiceFriendlyView;
@class PopulateServiceFriendlyView;
 ...
 *  菜单展现的动画类型
 */
//: typedef NS_ENUM(NSInteger, PopulateServiceFriendlyViewAnimateType) {
typedef NS_ENUM(NSInteger, PopulateServiceFriendlyViewAnimateType) {
    /**
     *  scale based the point of upper right corner.
     *  以右上角为基点进行伸缩
     */
    //: PopulateServiceFriendlyViewAnimateType_ScaleBasedTopRight = 0, 
    PopulateServiceFriendlyViewAnimateType_ScaleBasedTopRight = 0, //default
    /**
     *  scale based the point of upper left corner.
     *  以左上角为基点进行伸缩
     */
    //: PopulateServiceFriendlyViewAnimateType_ScaleBasedTopLeft,
    PopulateServiceFriendlyViewAnimateType_ScaleBasedTopLeft,
    /**
     *  scale based the middle point
     *  以中点为基点进行伸缩
     */
    //: PopulateServiceFriendlyViewAnimateType_ScaleBasedMiddle,
    PopulateServiceFriendlyViewAnimateType_ScaleBasedMiddle,
    /**
     *  fade in fade out
     *  淡入淡出效果
     */
    //: PopulateServiceFriendlyViewAnimateType_FadeInFadeOut,
    PopulateServiceFriendlyViewAnimateType_FadeInFadeOut,
    /**
     *  animate like RollerShutter
     *  卷帘效果
     */
    //: PopulateServiceFriendlyViewAnimateType_RollerShutter,
    PopulateServiceFriendlyViewAnimateType_RollerShutter,
    /**
     *  fall from top
     *  从上往下落下
     */
    //: PopulateServiceFriendlyViewAnimateType_FallFromTop,
    PopulateServiceFriendlyViewAnimateType_FallFromTop,
//: };
};


//: @protocol PopulateServiceFriendlyViewDelegate <NSObject>
@protocol PopulateServiceFriendlyViewDelegate <NSObject>
...
//: - (void)PopulateServiceFriendlyView:(PopulateServiceFriendlyView *)menuView WillAppearMenuCell:(PopulateServiceFriendlyBasedCell *)menuCell index:(NSInteger)index;
- (void)endlessNow:(PopulateServiceFriendlyView *)menuView betweenSpirit:(PopulateServiceFriendlyBasedCell *)menuCell thread:(NSInteger)index;

//: - (void)PopulateServiceFriendlyViewWillAppear;
- (void)underTransformOrbit;

//: - (void)PopulateServiceFriendlyViewWDidAppear;
- (void)skillEpisode;

//: - (void)PopulateServiceFriendlyViewWillDisappear;
- (void)announcementFactory;

//: - (void)PopulateServiceFriendlyViewWDidDisappear;
- (void)forgetBy;
 ...
 *  下拉菜单
 */
//: @interface PopulateServiceFriendlyView : UIView
@interface PopulateServiceFriendlyView : UIView
 ...
 *
 *
 *  @param menuModelsArray     下拉菜单模型数组,数组中存放着(PopulateServiceFriendlyModel模型对象)
 ...
 *
 *  @param menuModelsArray     drop-down menu models array. Array storage PopulateServiceFriendlyModel instances
 ...
 *
 */
//: @property (nonatomic, weak) id<PopulateServiceFriendlyViewDelegate> delegate;
@property (nonatomic, weak) id<PopulateServiceFriendlyViewDelegate> carefulSlipsed;
...
@property (nonatomic, assign) CGSize fuseEquipment;
...
@property (nonatomic, assign) CGFloat accelerateLand;
...
@property (nonatomic, assign) CGFloat occasion;
...
@property (nonatomic, assign) CGFloat surgeKick;
...
@property (nonatomic, assign) CGFloat expose;
...
@property (nonatomic, assign) CGFloat writingLeadMine;

/** 2、cell的类名, 必须是PopulateServiceFriendlyBasedCell的子类
 *               若用框架自带的cell,直接传@"PopulateServiceFriendlyCell"
 *               若使用自定义的cell,就传自定义cell的类名,若cell是xib,则传@"类名.xib"
 *                     举例如下:
 *                            ----->非xibcell   举例:@"YourCustomMenuCell"
 *                            ----->xib的cell   举例:@"YourCustomMenuCell.xib"
 *
 *               若不设置，默认为 @"PopulateServiceFriendlyCell"
 *
 *     -----------------------------------------------------------
 *     English description:
 *     -----------------------------------------------------------
 *     drop-down nemu cell's class name. Must be a subclass of PopulateServiceFriendlyBasedCell
 *              If want to use this framework's drop-down nemu cell, direct pass @"PopulateServiceFriendlyCell"
...
@property (nonatomic, assign) CGSize grain;
...
@property (nonatomic, assign) CGFloat merit;

/** 15、菜单的弹出的动画效果类型(若不设置，默认为PopulateServiceFriendlyViewAnimateType_ScaleBasedTopRight)
 *     -----------------------------------------------------------
 *     English description:
 *     -----------------------------------------------------------
 *     show drop-down menu animate type. (if not assign, default value is PopulateServiceFriendlyViewAnimateType_ScaleBasedTopRight)
 */
//: @property (nonatomic, assign) PopulateServiceFriendlyViewAnimateType menuAnimateType;
@property (nonatomic, assign) PopulateServiceFriendlyViewAnimateType skirt;
...
@property (nonatomic, strong) UIColor *easy;
...
@property (nonatomic, strong) UIColor *calm;
...
@property (nonatomic, assign) CGFloat drawer;

///==================================================================
///                    创建下拉菜单方式2
///   适用于自定义下拉菜单样式，单个属性进行赋值
///   >>步骤:
///   1、 [PopulateServiceFriendlyView alloc] init]
///   2、 对需要赋值的属性进行赋值
...
@property (nonatomic, assign) UIColor *connectionMineGrave;
...
@property (nonatomic, assign) BOOL thinSub;
...
/** 1、下拉菜单模型数组(数组里面存放 PopulateServiceFriendlyBasedModel的子类的对象) 
 *     -----------------------------------------------------------
 *     English description:
 *     -----------------------------------------------------------
 *     drop-down menu models array(Array storage PopulateServiceFriendlyModel instances or the PopulateServiceFriendlyModel's subclass instances )
 */
//: @property (nonatomic, strong) NSArray *menuModelsArray;
@property (nonatomic, strong) NSArray *transformComplex;
