...
// __CLOSE_PRINT__
//
//  UIActionSheet+DigestYardHeadGuard.m
//  eim_iphone
//
...
static char colorBraveKey;


//: @implementation UIActionSheet (DigestYardHeadGuard)
@implementation UIActionSheet (DigestYardHeadGuard)
...
- (void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    ...
    ActionSheetBlock block = [objc_getAssociatedObject(self, &colorBraveKey) copy];
    //: objc_setAssociatedObject(self,&kUIActionSheetBlockAddress,nil,OBJC_ASSOCIATION_COPY);
    objc_setAssociatedObject(self,&colorBraveKey,nil,OBJC_ASSOCIATION_COPY);
...
- (void)put: (UIView *)view message: (ActionSheetBlock)block
{
    ...
    objc_setAssociatedObject(self,&colorBraveKey,block,OBJC_ASSOCIATION_COPY);
    ...
    else
    {
        ...
        UITabBar *tabbar = [self wellRecent];
        //: if (tabbar)
        if (tabbar)
        {
        ...
        else
        {
            ...
            [self performSelector:@selector(treed:)
                       //: withObject:view
                       withObject:view
                       //: afterDelay:1];
                       afterDelay:1];
