...
#import "AtTryQuill.h"
@interface FocusedUpdateCloneFlash ()<NIMUserManagerDelegate,StepAmbientHeroicDelegate,MomentNegateMapperDelegate,LevelCornerDensePinDelegate>
...
@property (nonatomic, strong) LevelCornerDensePinUserView *reprotView;
@property (nonatomic, strong) ThornCropNavigateDale *changeRemarksView;
...
@implementation FocusedUpdateCloneFlash
...
//    [self setUpNav];
    ...
    [LocalizeElevateModest getWithUrl:Server_user_detail params:dict isShow:NO success:^(id responseObject) {
...
//    [ScaffoldOntoOrchestrate show];
    ...
    [LocalizeElevateModest getWithUrl:Server_group_getgroups params:nil isShow:NO success:^(id responseObject) {
        [self.loadingView animationClose];
//        [ScaffoldOntoOrchestrate dismissWithCompletion:^{
            NSDictionary *resultDict = (NSDictionary *)responseObject;
            NSString *code = [resultDict newStringValueForKey:@"code"];
            if (code.integerValue == 0) {
                ...
                StepAmbientHeroic *dataPick = [[StepAmbientHeroic alloc] initWithDelegate:self dataDict:dataDict selectedDict:nil jsonNode:@"name"];
                dataPick.tag = 500;
                [dataPick show];
            }
//        }];
...
#pragma mark - StepAmbientHeroicDelegate
...
-(void)sendAddFriendRequest{
    NSString *tempVerificationInfo = [SearchWindowAutosavePlot standardUserDefaults].tempVerificationInfo;
...
-(void)setingGroup{
    ...
    [LocalizeElevateModest getWithUrl:Server_group_addgroupuser params:dict isShow:YES success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
...
- (ThornCropNavigateDale *)changeRemarksView
{
    if(!_changeRemarksView){
        _changeRemarksView = [[ThornCropNavigateDale alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
{
    if(!_loadingView){
        _loadingView = [[ArchitectureAbovePetalWhiteWeave alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
    return  _loadingView;
}
- (LevelCornerDensePinUserView *)reprotView
{
    if(!_reprotView){
        _reprotView  = [[LevelCornerDensePinUserView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
