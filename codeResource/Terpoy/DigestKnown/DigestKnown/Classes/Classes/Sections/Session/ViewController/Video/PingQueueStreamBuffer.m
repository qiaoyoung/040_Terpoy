...
@property (nonatomic, strong) RefreshReceiveMomentumCoalesce *videoSlider;  //滑动条
...
- (RefreshReceiveMomentumCoalesce *)videoSlider{
    if (!_videoSlider) {
        _videoSlider = [[RefreshReceiveMomentumCoalesce alloc] initWithFrame:CGRectMake(0, 0, 3*(SCREEN_WIDTH/4), 20)];
...
{
    if(!_loadingView){
        _loadingView = [[ArchitectureAbovePetalWhiteWeave alloc]initWithFrame:CGRectMake(0, SCREEN_TOP_HEIGHT, SCREEN_WIDTH, SCREEN_HEIGHT-SCREEN_TOP_HEIGHT-SCREEN_TABBAR_HEIGHT)];
    }
    return  _loadingView;
}
