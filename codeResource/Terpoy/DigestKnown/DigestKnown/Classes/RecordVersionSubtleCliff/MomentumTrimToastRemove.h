...
//: @interface MomentumTrimToastRemove : UIViewController<UITableViewDataSource,UITableViewDelegate,NIMLoginManagerDelegate,NIMConversationManagerDelegate>
@interface MomentumTrimToastRemove : UIViewController<UITableViewDataSource,UITableViewDelegate,NIMLoginManagerDelegate,NIMConversationManagerDelegate>
/**
 *  会话列表tableView
 */
...
@property (nonatomic,strong) NSMutableArray * theSharp;

/**
 *  删除会话时是不是也同时删除服务器会话 (防止漫游)
 */
//: @property (nonatomic,assign) BOOL autoRemoveRemoteSession;
@property (nonatomic,assign) BOOL silentSpirit;

//: @property (nonatomic,strong) UITableView *tableView;
@property (nonatomic,strong) UITableView *extended;
 ...
 *  @return 时间戳格式化后的字符串
 *
 *  @discussion 默认实现为：最后一条消息的时间戳，具体时间戳消息格式详见NumberJungleEntity中， + (NSString*)showTime:(NSTimeInterval) msglastTime showDetail:(BOOL)showDetail 方法的实现。
 */
...
- (void)today:(NIMRecentSession *)recent
             //: atIndexPath:(NSIndexPath *)indexPath;
             verse:(NSIndexPath *)indexPath;
//: @end
@end