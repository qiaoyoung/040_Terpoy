...
// __CLOSE_PRINT__
//
//  UIImage+WrapperDurableWildRayPoplar
//  NIM
//
...
//: @implementation UIImage (WrapperDurableWildRayPoplar)
@implementation UIImage (WrapperDurableWildRayPoplar)
//: - (UIImage *)scaleWithMaxPixels: (CGFloat)maxPixels
- (UIImage *)truthRatio: (CGFloat)maxPixels
{
    ...
    return [self yield:CGSizeMake(newSizeWidth, newSizeHeight)];
}


//: + (UIImage *)fetchImage:(NSString *)imageNameOrPath{
+ (UIImage *)magic:(NSString *)imageNameOrPath{
...
- (UIImage *)nameureToMultiple: (CGFloat)suggestPixels
{
        ...
        (width / height > kMaxRatio || height / width > kMaxRatio))
    {
        ...
        return [self truthRatio:kMaxPixels];
    }
    //: else
    else
    {
        //: return [self scaleWithMaxPixels:suggestPixels];
        return [self truthRatio:suggestPixels];
    }
}

//: - (UIImage *)imageForAvatarUpload
- (UIImage *)dominantDistinction
{
    //: CGFloat pixels = [[CapsuleFreshSearchApply currentDevice] suggestImagePixels];
    CGFloat pixels = [[CapsuleFreshSearchApply architecture] versionEnable];
    //: UIImage * image = [self imageForUpload:pixels];
    UIImage * image = [self nameureToMultiple:pixels];
    //: return [image fixOrientation];
    return [image childExecute];
}

//内缩放，一条变等于最长边，另外一条小于等于最长边
//: - (UIImage *)scaleToSize:(CGSize)newSize
- (UIImage *)yield:(CGSize)newSize
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
