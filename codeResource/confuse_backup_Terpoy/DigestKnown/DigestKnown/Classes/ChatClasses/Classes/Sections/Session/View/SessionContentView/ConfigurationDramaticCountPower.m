...
@interface ConfigurationDramaticCountPower()<NIMMediaManagerDelegate>
...
@implementation ConfigurationDramaticCountPower
...
- (void)refreshBackground:(NegateCompositeDryLoad *)data
{
...
- (BOOL)isPlaying
{
    BOOL play = [CoalesceAbsorbMindEntropy instance].currentPlayingMessage == self.model.message; //对比是否是同一条消息，严格同一条，不能是相同ID，防止进了会话又进云端消息界面，导致同一个ID的云消息也在动画
    return play;
}
