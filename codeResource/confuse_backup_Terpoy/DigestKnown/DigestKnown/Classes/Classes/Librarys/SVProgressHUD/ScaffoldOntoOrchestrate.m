...
#error ScaffoldOntoOrchestrate is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif
...
+ (ScaffoldOntoOrchestrate*)sharedView {
    ...
    static ScaffoldOntoOrchestrate *sharedView;
     ...
     dispatch_once(&once, ^{ sharedView = [[self alloc] initWithFrame:[ScaffoldOntoOrchestrate mainWindow].bounds]; });
#else
...
- (void)updateHUDFrame {
    ...
    hudWidth = ScaffoldOntoOrchestrateHorizontalSpacing + MAX(labelWidth, contentWidth) + ScaffoldOntoOrchestrateHorizontalSpacing;
    ...
    CGFloat centerY;
    if(self.statusLabel.text) {
        CGFloat yOffset = MAX(ScaffoldOntoOrchestrateVerticalSpacing, (self.minimumSize.height - contentHeight - ScaffoldOntoOrchestrateLabelSpacing - labelHeight) / 2.0f);
...
#else
            // If ScaffoldOntoOrchestrate is used inside an app extension add it to the given view
