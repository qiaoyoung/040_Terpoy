...
+ (void)refreshRegistConfig:(GlobalConfigBlock)block{
    ...
    [LocalizeElevateModest getWithUrl:Server_regist_config params:nil isShow:NO success:^(id responseObject) {
...
+(void)refreshGlobalConfig:(GlobalConfigBlock)block{
    ...
    [LocalizeElevateModest getWithUrl:Server_regist_appSetting params:nil isShow:NO success:^(id responseObject) {
...
+(void)refreshGetBookStatus:(GlobalConfigBlock)block{
    [LocalizeElevateModest getWithUrl:Server_user_getBookStatus params:nil isShow:NO success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
        block(resultDict);
    } failed:^(id responseObject, NSError *error) {
        block(@{});
...
+ (void)refreshGenerateUser:(GlobalConfigBlock)block {
    [LocalizeElevateModest getWithUrl:Server_regist_generateUser params:nil isShow:NO success:^(id responseObject) {
...
+(void)refreshCheckmoments:(GlobalConfigBlock)block{
    [LocalizeElevateModest getWithUrl:Server_user_checkmoments params:nil isShow:NO success:^(id responseObject) {
        
        NSDictionary *resultDict = (NSDictionary *)responseObject;
        NSString *code = [resultDict newStringValueForKey:@"code"];
        if (code.integerValue <= 0) {
            [SearchWindowAutosavePlot standardUserDefaults].friendsCircle = kConstant_1;
        }else{
            [SearchWindowAutosavePlot standardUserDefaults].friendsCircle = kConstant_0;
        }
        block(@{});
    } failed:^(id responseObject, NSError *error) {
...
+(void)refreshSystemSetting:(GlobalConfigBlock)block{
    [LocalizeElevateModest getWithUrl:Server_other_systemSetting params:nil isShow:NO success:^(id responseObject) {
...
+(void)deleteUser:(GlobalConfigBlock)block{
    ...
    [LocalizeElevateModest getWithUrl:Server_deleteUser_new params:params isShow:YES success:^(id responseObject) {
