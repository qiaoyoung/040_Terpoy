    ...
    if (_tableView == nil) {
        ...
        if ([self.cellClassName hasSuffix:@".xib"]) { //xib名称
            ...
            if (![NSClassFromString(className) isSubclassOfClass:[PopulateServiceFriendlyBasedCell class]]) {
                FFLog(@"%@这个类不是PopulateServiceFriendlyBasedCell的子类,请继承这个类",className);
                return _tableView;
            }
            ...
            if (![NSClassFromString(self.cellClassName) isSubclassOfClass:[PopulateServiceFriendlyBasedCell class]]) {
                FFLog(@"%@这个类不是PopulateServiceFriendlyBasedCell的子类,请继承这个类",self.cellClassName);
                return _tableView;
            }
...
- (void)setMenuAnimateType:(PopulateServiceFriendlyViewAnimateType)menuAnimateType { //15
