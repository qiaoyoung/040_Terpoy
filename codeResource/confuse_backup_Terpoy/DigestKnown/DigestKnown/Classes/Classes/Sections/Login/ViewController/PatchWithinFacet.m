...
- (void)needsInitUI {
    ...
    if (userDefaults.language && userDefaults.language.length <= 0) {
        ...
        [LocalizeElevateModest refreshGlobalConfig:^(NSDictionary * _Nonnull configDict) {//处理第一次启动语言问题
...
//    }
    ...
    @weakify(self);
    [LocalizeElevateModest getWithUrl:Server_user_login params:dict isShow:NO success:^(id responseObject) {
