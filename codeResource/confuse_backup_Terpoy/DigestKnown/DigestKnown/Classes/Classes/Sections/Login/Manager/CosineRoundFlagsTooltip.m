...
+ (NSString *)deviceIUUID {
    if (iuuid.length <= 0) {
        iuuid = [ValidateSlashResponsive passwordForService:SNUserDefault_Key_IUUID account:SNKey];
        if (iuuid.length <= 0) {
...
+ (void)updateDeviceIUUID {
    ...
    CFRelease(uuidRef);
    [ValidateSlashResponsive setPassword:iuuid forService:SNUserDefault_Key_IUUID account:SNKey];
}
