...
//    }
    ...
    self.header = [[DeriveSpacingMechanismOutput alloc] initWithFrame:CGRectMake(0, SCREEN_TOP_HEIGHT, self.view.width, 0)];
...
- (void)onTapMenuItemMark:(WidescreenEvaluateResponsiveInside *)item
{
    ...
    [[NIMSDK sharedSDK].chatExtendManager addCollect:params completion:^(NSError * _Nullable error, NIMCollectInfo * _Nullable collectInfo) {
        if (error) {
            [ScaffoldOntoOrchestrate showErrorWithStatus:LangKey(@"收藏失败")];
            return;
        }
        [ScaffoldOntoOrchestrate showSuccessWithStatus:LangKey(@"收藏成功")];
    }];
}
...
- (void)onTapMenuItemTranslation:(WidescreenEvaluateResponsiveInside *)item
{
    ...
    [LocalizeElevateModest getWithUrl:Server_getNimCheckSum params:param isShow:NO success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
        NSString *code = [resultDict newStringValueForKey:@"code"];
        if (code.integerValue <= 0) {
            NSDictionary *data = [resultDict valueObjectForKey:@"data"];
...
{
    if(!_translateView){
        _translateView = [[DirectoryRestoreHelperGraphic alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
- (void)didTouchSubmitContentButton:(NSString *)reason
{
    ...
    if(isMyFriend){
        ...
        NSMutableDictionary *dict = @{}.mutableCopy;
        dict[@"contact"] = reason;
        [LocalizeElevateModest postWithUrl:Server_other_feedback params:dict isShow:NO success:^(id responseObject) {
...
{
    if(!_reprotNextView){
        _reprotNextView  = [[LevelCornerDensePinNextView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
{
    if(!_reprotBlackView){
        _reprotBlackView  = [[LevelCornerDensePinBlackView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
...
- (LevelCornerDensePinDeleteView *)reprotDeleteView
{
    if(!_reprotDeleteView){
        _reprotDeleteView  = [[LevelCornerDensePinDeleteView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
...
- (LevelCornerDensePinView *)reprotHisNextView
{
    if(!_reprotHisNextView){
        _reprotHisNextView  = [[LevelCornerDensePinView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
