...
@property (nonatomic, strong) PopulateServiceFriendlyView *dropdownMenu;/** 下拉菜单 */
...
-(void)checkCreateTeam{
    ...
    [LocalizeElevateModest getWithUrl:Server_check_createteam params:nil isShow:NO success:^(id responseObject) {
...
- (NSArray *)getMenuModelsArray {    
    ...
    PopulateServiceFriendlyModel *menuModel1 = [PopulateServiceFriendlyModel ff_DropDownMenuModelWithMenuItemTitle:activity menuItemIconName:@"ic_create_chat" menuBlock:^{
        if (_creatTeam.integerValue != 0) {
            [ScaffoldOntoOrchestrate showMessage:LangKey(@"please_contact_your_administrator")];
        }else{
    ...
    //菜单模型1
    @weakify(self);
    PopulateServiceFriendlyModel *menuModel3 = [PopulateServiceFriendlyModel ff_DropDownMenuModelWithMenuItemTitle:LangKey(@"fragment_contact_new_scan") menuItemIconName:@"ic_scan" menuBlock:^{
        @strongify(self);
        [self requestAuthorizationForVideo];
    }];
...
-(void)newGroupSyncRequest:(NSString *)groupName teamID:(NSString *)teamId{
    ...
    dict[@"name"] = groupName;
    dict[@"id"] = teamId;
    [LocalizeElevateModest getWithUrl:Server_team_create params:dict isShow:NO success:^(id responseObject) {
...
{
    if(!_noticeView){
        _noticeView = [[MelodyRemoveConnectEntropy alloc]initWithFrame:CGRectMake(0, SCREEN_STATUS_HEIGHT+140, SCREEN_WIDTH, SCREEN_SAFE_HEIGHT-140)];
